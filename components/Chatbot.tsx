      ...
      setMessages([
        {
          ...
          text: `Ola ${
            currentUser?.name || ""
          }! Como posso ajudar no ClubsMaker hoje?`,
        },
      ]);
    }
  }, [isOpen, currentUser]);
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 top-12 w-80 sm:w-96 h-[500px] bg-white rounded-xl shadow-2xl flex flex-col z-50 border border-stone-200 overflow-hidden animate-fade-in-down origin-top-right">
            {/* Cabeçalho */}
            <div className="bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-secondary)] text-white p-4 flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/80">ClubsMaker AI</p>
                </div>
              </div>
              <button
                onClick={() => setIsOpen(false)}
            ...
            >
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-2 bg-stone-100 border-0 rounded-full focus:ring-2 focus:ring-[var(--color-primary)] focus:bg-white transition-all text-sm"
                  disabled={isLoading}
                />
                <button
                  type="submit"
                  className="bg-[var(--color-secondary)] text-white p-2 rounded-full hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm"
