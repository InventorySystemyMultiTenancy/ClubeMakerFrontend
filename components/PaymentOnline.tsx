...
export default function PaymentOnline(props: PaymentOnlineProps) {
  ...
  const {
    ...
    total,
    items,
    userEmail = "cliente@clubsmaker.com",
    userName = "Cliente",
    onSuccess,
    onError,
  } = props;
      ...
      <div className="mx-auto max-w-md rounded-2xl border border-blue-500/20 bg-[#071226] p-8 text-white shadow-xl shadow-blue-950/25">
        ...
        <div className="space-y-4">
          <button
            ...
            className="w-full bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-secondary)] hover:from-[var(--color-primary-hover)] hover:to-red-700 text-white font-bold py-4 px-6 rounded-xl transition-all shadow-lg hover:shadow-xl flex items-center justify-between"
          >
        ...
        {showPaymentStatus && paymentStatusMsg && (
          <div className="mt-6 flex flex-col items-center">
            <div
              className={
                boxColor === "green"
                  ? "bg-[var(--color-success)] text-white font-bold px-6 py-4 rounded-xl shadow-lg text-center text-lg mb-4 flex flex-col items-center"
                  : "bg-[var(--color-accent)] text-[var(--color-dark)] font-bold px-6 py-4 rounded-xl shadow-lg text-center text-lg animate-pulse mb-4"
              }
              style={{
                boxShadow:
                  boxColor === "green"
            ...
            <button
              className="bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] text-white font-bold py-2 px-6 rounded-xl shadow-md transition-all"
              onClick={() => {
