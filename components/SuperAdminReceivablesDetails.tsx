...
> = ({
  ...
  return (
    <div className="bg-white shadow-xl rounded-2xl p-3 sm:p-6 border-2 border-[var(--color-primary-light)] mt-4 sm:mt-8 overflow-hidden">
      <h2 className="text-3xl sm:text-2xl font-bold text-[var(--color-primary-active)] mb-4 leading-tight break-words">
      ...
      <div className="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        {safeOrders.length > 0 && (
          <button
            className="w-full sm:w-auto px-3 py-2 bg-[var(--color-primary)] text-white rounded font-bold text-xs hover:bg-[var(--color-primary-hover)] transition"
      ...
      {safeOrders.map((order) => (
        <div
          key={order.id}
          className={`order-card border rounded-lg p-3 sm:p-4 mb-4 sm:mb-6 ${receivedOrderIds.includes(order.id) ? "bg-green-100 border-green-400" : "bg-[var(--color-primary-lighter)]"}`}
        >
          <div className="mb-2 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
            <div className="flex items-start gap-2 min-w-0">
              <input
                ...
                className="accent-[var(--color-primary)] w-5 h-5 mt-1 shrink-0"
              />
            ...
            <button
              className="self-start px-2 py-1 bg-[var(--color-secondary)] text-white rounded text-xs font-bold hover:bg-red-700 transition shrink-0"
              onClick={() => {
                const backendUrl =
